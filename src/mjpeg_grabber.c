...
static int extract_one_jpeg_from_acc(void) {
    // search SOI
    size_t i = 0;
    while (i + 1 < acc_sz && !(acc[i] == 0xFF && acc[i+1] == 0xD8)) i++;
    if (i + 1 >= acc_sz) { // no SOI
        // discard all but last byte (in case it's 0xFF)
        if (acc_sz > 1) { memmove(acc, acc + acc_sz - 1, 1); acc_sz = 1; }
        return 0;
    }
    // drop leading bytes before SOI
    if (i > 0) { memmove(acc, acc + i, acc_sz - i); acc_sz -= i; }

    // search EOI from position 2 onward
    size_t j = 2;
    while (j + 1 < acc_sz && !(acc[j] == 0xFF && acc[j+1] == 0xD9)) j++;
    if (j + 1 >= acc_sz) return 0; // need more bytes
...
static void *reader_thread(void *arg) {
    ...
    for (;;) {
        ...
        if (acc_sz + (size_t)r > acc_cap) {
            ...
            while (new_cap < acc_sz + (size_t)r) new_cap *= 2;
...
// Start rpicam-vid -> stdout (pipe), parse MJPEG
int stream_start(int width, int height, int fps) {
    if (cam_fd != -1) return 0; // already started
    ...
    if (rc != 0) {
        perror("posix_spawnp rpicam-vid");
...
int grab_jpeg(const char *path) {
    ...
    if (latest && latest_sz > 0) {
        ...
        if (f) {
            if (fwrite(latest, 1, latest_sz, f) == latest_sz) rc = 0;
            fclose(f);
        }
    } else {
...
void stream_stop(void) {
    ...
    free(latest); latest = NULL; latest_sz = 0;
    free(acc); acc = NULL; acc_sz = acc_cap = 0;
}
...
int main(int argc, char **argv) {
    ...
    if (stream_start(w, h, f) != 0) die("stream_start");

    printf("Streaming %dx%d @ %dfps. Press Enter to save frame, Ctrl+C to quit.\n", w, h, f);
    int idx = 0;
    for (;;) {
        int c = getchar();
        if (c == EOF) break;
